...
| UI-11 | Phase Preview UI | UI/HUD | Medium | Done | UI-03, TIME-03, TIME-07, UI-10 | Artist / UI / Audio | M | • Hold P to peek next-tick enemy/hazard previews when Move Previews enabled; • hazard overlays only on upcoming state change; • previews hidden during slide, when P released, or setting off; • brief pulse when phase objects change on tick |
